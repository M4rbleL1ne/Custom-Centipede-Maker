...
public class CustomCentiBreedParams(CreatureTemplate.Type type, CreatureTemplate.Type? parent = null) : BreedParameters, ICloneable
{
    ...
    public void SetConfig(Configurable<int> config)
    {
        if (config?.key is not string s)
            return;
        config.Value = ComputeStringHash(s) switch
        {
    ...
    public void Set(Configurable<string> config)
    {
        ...
        switch (ComputeStringHash(s))
        {
            ...
            case K_Inherit:
                ...
                else if (tp.value == nameof(MoreSlugcats.MoreSlugcatsEnums.CreatureTemplateType.AquaCenti))
                {
                    _nonMSCAqua = true;
    ...
    public void SetColorType(Configurable<string> config)
    {
        if (config?.key is not string s)
            return;
        var val = config.Value;
        switch (ComputeStringHash(s.Replace("Type", string.Empty)))
        {
            case K_BodyBlackColor:
    ...
    public void SetAlpha(Configurable<float> config)
    {
        if (config?.key is not string s)
            return;
        var val = config.Value;
        switch (ComputeStringHash(s.Replace(s_Alpha, string.Empty)))
        {
            case K_BodyBlackColor:
    ...
    public void SetAlphaConfig(Configurable<float> config)
    {
        if (config?.key is not string s)
            return;
        config.Value = ComputeStringHash(s.Replace(s_Alpha, string.Empty)) switch
        {
    ...
    {
        var v = ComputeStringHash(fieldName);
        return v is K_KillScoreHidden
            or K_UsesUnlockData
