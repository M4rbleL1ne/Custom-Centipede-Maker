...
public sealed class CustomCentiPlugin : BaseUnityPlugin
{
    ...
    internal const string K_ID = "lb-fgf-m4r-ik.custom-centis", K_VERSION = "1.0.3";
    ...
    public static void On_RainWorld_PostModsInit(On.RainWorld.orig_PostModsInit orig, RainWorld self)
    {
        ...
        try
        {
            ...
            {
                var path = mods[i].path + "/CustomCentis";
                ...
                for (var j = 0; j < files.Length; j++)
                {
                        ...
                        for (var k = 0; k < lines.Length; k++)
                        {
                            ...
                            if (line.Length > 0 && line[0] != Comment)
                            {
                                ...
                                if (sAr.Length > 1)
                                {
                                    ...
                                    int s0Hash = ComputeStringHash(s0), s1Hash = ComputeStringHash(s1);
                                    if (k == 0 && s0Hash == K_Inherit)
                                    {
                                        if (s1Hash == CustomCentiTemplates.K_AquaCenti && !ModManager.MSC)
                                        {
                    ...
                    else if (fnm.EndsWith(".png"))
                    {
                        ...
                        {
                            var pth = "CustomCentis/" + fnm;
                            if (File.Exists(path + "/" + fnm + ".txt"))
